...
static char datestringBuffer[dt_MAX_STRING_LEN+1];  // must be big enough for longest string and the terminating null
...
{
	strcpy_P(datestringBuffer, (PGM_P)pgm_read_word(&(monthNames_P[month])));
...
char* monthShortStr(uint8_t month)
{
	for (int i=0; i < dt_SHORT_STR_LEN; i++)
		datestringBuffer[i] = pgm_read_byte(&(monthShortNames_P[i+ (month*dt_SHORT_STR_LEN)]));
	datestringBuffer[dt_SHORT_STR_LEN] = 0;
	return datestringBuffer;
}

char* dayStr(uint8_t day)
{
	strcpy_P(datestringBuffer, (PGM_P)pgm_read_word(&(dayNames_P[day])));
	return datestringBuffer;
}
...
char* dayShortStr(uint8_t day)
{
	...
	for (int i=0; i < dt_SHORT_STR_LEN; i++)
		datestringBuffer[i] = pgm_read_byte(&(dayShortNames_P[index + i]));
	datestringBuffer[dt_SHORT_STR_LEN] = 0;
	return datestringBuffer;
}