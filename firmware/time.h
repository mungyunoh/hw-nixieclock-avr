...
// made time_t available in global.h
...
uint8_t		timeIsAM(time_t t);    // returns true the given time is AM
uint8_t		timeIsPM(time_t t);    // returns true the given time is PM
uint8_t		timeGetMinute(time_t t);  // the minute for the given time
uint8_t		timeGetSecond(time_t t);  // the second for the given time
uint8_t		timeGetDay(time_t t);     // the day for the given time
uint8_t		timeGetWeekday(time_t t); // the weekday for the given time
uint8_t		timeGetMonth(time_t t);   // the month for the given time
uint16_t	timeGetYear(time_t t);    // the year for the given time

time_t  timeNow(void);              // return the current time as seconds since Jan 1 1970
...
void timeBreak(time_t time, tmElements_t* el);  // break time_t into elements
time_t timeMake(tmElements_t el);  // convert time elements into time_t
